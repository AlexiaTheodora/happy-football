...
# all of the streams have been moved in the global definition so they can be manipulated when closing the game
'''global info_emg1,outlet_emg1,info_emg2,outlet_emg2,info_imu1,outlet_imu1,info_imu2,outlet_imu2
info_emg1 = None
outlet_emg1 = None
...
def detct_tty():
		print('################# Looking for ttys/ COM ports with connected Myo dongles ##########')
		for p in comports():
			if re.search(r'PID=2458:0*1', p[2]):
				print('Found Myo dongle on virtual serial port', p[0])
				#print('p[1]:', p[1])
				#print('p[2]:', p[2])
				yield p[0]  ### on Linux: e.g.: /dev/ttyACM0 or /dev/ttyACM1, on Windows, e.g.: COM3, COM5
		return None
    ...
    while pull_at_once == samps_pulled:
        ...
        if len(arr) > 0:
            ...
            if n_tries == max_tries:
                print ("Stream does not seem to provide any data.")
                ###stop_event.set() ### PF
...
class GameState:
    def __init__(self, screen, keyboard):
        ...
        self.close_button = Button(X - 555, Y - 550, 100, 50, "X")
        self.keyboard = keyboard
        self.myo_data = []
        self.type = ''
        self.process = liesl.Recorder()
        self.is_process_recording = False
    ...
    def close_myo_streams(self):
        stream_emg1 = resolve_stream('name', 'EMG-Left')
        stream_emg2 = resolve_stream('name', 'EMG-Right')
        stream_imu1 = resolve_stream('name', 'IMU-Left')
        stream_imu2 = resolve_stream('name', 'IMU-Right')
        outlet_emg1 = StreamOutlet(stream_emg1[0])
        outlet_emg2 = StreamOutlet(stream_emg2[0])
        outlet_imu1 = StreamOutlet(stream_imu1[0])
        outlet_imu2 = StreamOutlet(stream_imu2[0])
        outlet_emg1.__del__()
        outlet_emg2.__del__()
        outlet_imu1.__del__()
        outlet_imu2.__del__()
        print("Myo streams closed")

    def quit_game(self, stop_event):
        if FILE != None:
        ...
        if self.is_process_recording:
            stop_lab_recorder(self.process)
            self.is_process_recording = False
        print(stop_event)
        stop_event.set()
        print("Stop event is set:", stop_event.is_set())
        print("closing the marker stream")
        self.close_myo_streams()
        outlet_markers.__del__()
        print("saving files and quiting")
    ...
    def intro(self, stop_event, play, back=False):
        if back:
            pygame.display.flip()
        ...
        while not (self.start_button.clicked or self.training_button.clicked or self.yes_no_button.clicked or self.close_button.clicked):
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.quit_game(stop_event)
                ...
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                    ...
                    if self.close_button.rect.collidepoint(event.pos):
                        self.close_button.clicked = True
                        self.intro_done = True
                        print("Printing after clicking on button ",stop_event)
            pygame.display.flip()

        if self.start_button.clicked:
            self.source_directory = generate_folder("Game")
            self.start_play(stop_event, play)
        ...
        if self.training_button.clicked:
            ...
            self.training.intro_training_function(stop_event,play,False)
        if self.yes_no_button.clicked:
            self.source_directory = generate_folder("Yes_No")
            self.start_play(stop_event,play,yes_no=True)

        if self.close_button.clicked:
            self.quit_game(stop_event)
            play.set()
            print("After quiting the game, closing everything, IS THE PLAY SET", play.is_set())
    ...
    def start_play(self,stop_event,play, training_mode=False, yes_no=False):
        ...
        self.is_process_recording = True
        ...
        while not self.play_done:
            ...
            if self.back_button.clicked:
                ...
                self.intro(stop_event,play, back=True)
            pygame.display.flip()
...
class Training:
    ...
    def intro_training_function(self,stop_event, play, back=False):
        if back:
            pygame.display.flip()
        ...
        while not (
            ...
            for event in pygame.event.get():
                if event.type == pygame.QUIT:
                    self.game_state.quit_game(stop_event)
                if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
        ...
        self.game_state.start_play(stop_event,play,training_mode=True, yes_no=False)
...
def main_game(stop_event):
    ...
    play = multiprocessing.Event()
    # mio_connect.main(sys.argv[1:])

    while not play.is_set():
        game_state.intro(stop_event,play)
...
class MioConnect:
    ...
    def main(self, argv,stop_event, connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right):
        global CONNECTED
        ...
        try:
            ...
            # Connect
            myo_driver.run(connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right)
            ...
            while not stop_event.is_set():
                ...
                while not (myo_driver.data_handler.myo_imu_data.empty()):
                    ...
                    if (data_both_samples.get('emg')):
                        ...
                        elif (data_both_samples.get('emg').get("2")):           ### PF: needed if payload Connection-Id happens to be 2 instead of 0
                            #print("===get 2===")
                            emg1 = list(data_both_samples.get('emg').get("2"))
                    ...
                    if (data_both_samples.get('imu')):
                        ...
                        elif (data_both_samples.get('imu').get("2")):               
                            imu1 = list(data_both_samples.get('imu').get("2"))
            ...
            print ("Terminating the game due to a critical error")
            outlet_emg1.__del__()
            outlet_emg2.__del__()
            outlet_imu1.__del__()
            outlet_imu2.__del__()
            ...
            # The process must be attached to a console and in the same process group to receive CTRL_C_EVENT
                os.kill(os.getpid(), signal.CTRL_C_EVENT)
            else:
                print("CTRL_C_EVENT not supported on this platform. Exiting with SIGINT.")
            ...
            print ("handling KeyboardInterrupt: terminating")
...
class MyoDriver:
    ...
    def run(self, connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right):
        """
        ...
        while len(self.myos) < self.config.MYO_AMOUNT:
            ...
            print()
        '''
        self.add_myo_connection(connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right)
        self.add_myo_connection(connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right)
        self.receive()
    ...
    def add_myo_connection(self, connected1, connected2,myo_A_left ,myo_B_left, myo_A_right, myo_B_right):
        """
        ...
        # Add handlers
        self.bluetooth.add_connection_status_handler(
        self.create_connection_status_handle(self.myo_to_connect, connected1, connected2, myo_A_left ,myo_B_left, myo_A_right, myo_B_right))
    ...
    def connect_and_retry(self, myo, timeout=None, max_retries=None):
        ...
        #print(f"Myo connect-id BEFORE Retry: {myo.connection_id}")    ### is always None
    ...
    def create_disconnect_handle(self, myo):
        def handle_disconnect(_, payload):
            ...
            if myo.connection_id == payload['connection'] :
                ...
                if myo.connection_id is None:
                    print("Myo connection id is none; terminating the game")
                    stop_event.set()   ### or just pass?
                
                elif payload['connection'] != myo.connection_id:  ### PF
                    print ("===connection id mismatch===")
                    print(f" === Myo.connection-id {myo.connection_id}  VS payload-connection-id {payload['connection']} ===")
                    print("terminating the game")
                    #raise KeyboardInterrupt("Terminating due to critical error")  ### does not work here
                    stop_event.set()  ### or just pass?
    ...
    def create_connection_status_handle(self, myo, connected1, connected2, myo_A_left ,myo_B_left, myo_A_right, myo_B_right):
        def handle_connection_status(_, payload):
            ...
            if payload['address'] == myo.address and payload['flags'] == 5:
                ...
                myo.set_connected(True) ###  PF: here?!
                myo.set_id(payload['connection'])

                if  (payload['connection'] == 0 or payload['connection'] == 2):
                    print(f"===payload['connection'] {payload['connection']}===")
                    print("left side")
                    connected1.set()
                    if myo.mac_address == Config.MAC_ADDR_MYO_B:
                        print("myo_B left set")
                        myo_B_left.set()
                    elif myo.mac_address == Config.MAC_ADDR_MYO_A:
                        myo_A_left.set()                        
                        print("myo_A left set")
                    print("==========================")
                elif payload['connection'] == 1:
                    print("===payload['connection'] 1 ===")
                    print("right side")
                    connected2.set()
                    if myo.mac_address == Config.MAC_ADDR_MYO_B:
                        myo_B_right.set()
                        print("myo_B right set")
                    elif myo.mac_address == Config.MAC_ADDR_MYO_A:
                        myo_A_right.set()
                        print("myo_A right set")
                    print("==========================")

                else:
                    print(f"===payload['connection'] = {payload['connection']}:  we need to terminate the game ===")
    ...
    def deep_sleep_all(self):
        ...
        for m in self.myos:
            print('Following id of myo connection is disconnecting:',m.connection_id)
...
class Config:
    ...
    GET_MYO_INFO = True  # Get and display myo info at sync  ### PF: if set to False, the game works but also Battery level will not be read out!!
    ...
    # addresses of Swiss Myos:
    #MAC_ADDR_MYO_A ='e8-26-3b-f2-38-16'    ### labeled with LINKS, shall be changed to A
    #MAC_ADDR_MYO_B ='ea-de-bf-42-2f-30'    ### labeled with RECHTS, shall be changed to B
    
    # addresses of development/lab Myos
    MAC_ADDR_MYO_A = 'ec-07-35-d9-7a-c6'    ### labeled with #2
    MAC_ADDR_MYO_B = 'd7-5a-4d-ad-ba-0f'    ### labeled with #5


# always left MAC_ADDR_MYO_A
# always right - MAC_ADDR_MYO_B
...
if __name__ == "__main__":
    ...
    # Ensure the script is in a context where SIGINT can be received    ### PF
    signal.signal(signal.SIGINT, lambda signum, frame: sys.exit("Terminated via SIGINT (Ctrl+C)"))

    pygame.init()

    tty_generator = detct_tty()  
    try:
        tty_a = next(tty_generator)
        #print (f'tty_a = {tty_a}')
    except StopIteration:
        #print('No Myo dongle found!')
        raise ValueError('No Myo dongle found! Please connect the 2 Myo dongles to USB ports')
       
    try:
        tty_b = next(tty_generator)
        #print (f'tty_b = {tty_b}')
    except StopIteration:
        #raise ValueError('2nd Myo dongle NOT found! Please connect the 2nd Myo dongle to a USB port')
        print('2nd Myo dongle NOT found!')
    ...
    text_nr_myo = None
    #text_nr_myo_other = None
    is_number2_myo_left = False
    is_number5_myo_left = False
    is_number2_myo_right = False
    is_number5_myo_right = False

    text_nr_myo_complementary = None
    ...
    process_mio_connect = Process(target=mio_connect.main, args=('sys.argv[1:]',stop_event, connected1, connected2, \
                                    myo_A_left ,myo_B_left, myo_A_right, myo_B_right))
    process_game = Process(target=main_game, args=(stop_event,))
    #print("Ïs the stop event set", stop_event.is_set())
    ...
    try:
                    ...
                    if event.type == pygame.MOUSEBUTTONDOWN and event.button == 1:
                        if connect_button.rect.collidepoint(event.pos):
                            ...
                            while (not myo_connected1 or not myo_connected2):
                                ...
                                if connected1.wait(10) and not myo_connected1 :

                                    if myo_A_left.wait(3):
                                        text_nr_myo = FONT.render("Nr A", True, WHITE)
                                    elif myo_B_left.wait(3):
                                        text_nr_myo = FONT.render("Nr B", True, WHITE)
                                    else:
                                        print ("myoNr-event for left not yet set")
                                    text = FONT.render(translate.get('Translate', 'myo.left.connected'), True, WHITE)
                                    text_rect = text.get_rect()
                                    text_rect.center = (X - 400, Y)
                                    screen.blit(text, text_rect)

                                    text_rect = text_nr_myo.get_rect()
                                    text_rect.center = (X - 400, Y + 30)
                                    screen.blit(text_nr_myo, text_rect)
                                  
                                    pygame.display.flip()
                                    myo_connected1 = True

                                if connected2.wait(10) and not myo_connected2:

                                    if myo_A_right.wait(3):
                                        text_nr_myo = FONT.render("Nr A", True, WHITE)
                                    elif myo_B_right.wait(3):
                                        text_nr_myo = FONT.render("Nr B", True, WHITE)
                                    else:
                                        print ("myoNr-event for RIGHT not yet set")
                                ...
                                then = time.time()
                                print (f"Duration was {then - now} seconds")
                                
                                #if not connected1.is_set() and not connected2.is_set():    ### PF: commented out - it makes problems if Myos are moved too late after pressing thr connect-button
                                #    play = False
                                #    stop_event.set()
        ...
        #print(f"finally process_game.is_alive(): {process_game.is_alive()}")           ### always True
        #print(f"process_mio_connect.is_alive(): {process_mio_connect.is_alive()}")     ### always True
        pygame.quit()
        sys.exit(0)
