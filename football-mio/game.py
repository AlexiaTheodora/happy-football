...
def save_data_stream(directory_path,stream_name):
    file_path = os.path.join(directory_path, f'{stream_name}.xdf')
    labrecorder_path = 'C:/Users/BAI/tools-local/LabRecorder/LabRecorder.exe'
    args = [
        labrecorder_path,
        '--filename', file_path
    ]
    subprocess.run(args)
    print(f"Stream data saved to {file_path}")
...
def generate_folder(type):
    ...
    # save the other files generated by LabRecorder todo
    save_data_to_path = path + "/" + today + "_" + type
    save_data_stream(save_data_to_path, outlet_markers)
